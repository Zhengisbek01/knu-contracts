import { useState, useMemo } from "react";
import { BarChart, Bar, LineChart, Line, PieChart, Pie, Cell, XAxis, YAxis, CartesianGrid, Tooltip, Legend, ResponsiveContainer } from "recharts";

// ─── KNU Brand Colors ─────────────────────────────────────────────────────────
const C = {
  navy:    "#1B2E6B",
  navyDk:  "#0F1E4A",
  navyMd:  "#243880",
  gold:    "#F5A800",
  goldLt:  "#FFD166",
  goldDm:  "#FFF0C2",
  white:   "#FFFFFF",
  off:     "#F7F9FF",
  slate:   "#8899BB",
  border:  "#DDE3F0",
  red:     "#E53935",
  green:   "#2E7D32",
  teal:    "#00796B",
};

const LOGO_B64 = "data:image/png;base64,";

const VAT = 0.12;
const MONTHS = ["Январь","Февраль","Март","Апрель","Май","Июнь","Июль","Август","Сентябрь","Октябрь","Ноябрь","Декабрь"];
const YEARS  = [2023,2024,2025,2026];

const fmt = n => new Intl.NumberFormat("ru-KZ",{style:"currency",currency:"KZT",maximumFractionDigits:0}).format(n);
const num = n => new Intl.NumberFormat("ru-KZ").format(Math.round(n));
const withVat    = x => x * (1 + VAT);
const withoutVat = x => x / (1 + VAT);
const vatOnly    = x => x - withoutVat(x);

// ─── KNU Logo (topbar mini) ───────────────────────────────────────────────────
function KNULogoMini() {
  return (
    <div style={{ display:"flex", alignItems:"center", gap:8, background:C.navy, borderRadius:9, padding:"5px 12px 5px 6px", border:`1px solid rgba(245,168,0,0.3)` }}>
      <img src={LOGO_B64} alt="KNU" style={{ height:28, width:"auto", objectFit:"contain" }}/>
      <div style={{ lineHeight:1.2 }}>
        <div style={{ color:C.gold, fontSize:9, fontWeight:800, letterSpacing:"0.1em" }}>KNU</div>
        <div style={{ color:"rgba(255,255,255,0.55)", fontSize:8, letterSpacing:"0.06em" }}>Nuclear Univ.</div>
      </div>
    </div>
  );
}

// ─── KPI Card ─────────────────────────────────────────────────────────────────
function KpiCard({ label, value, sub, sub2, accent, icon }) {
  return (
    <div style={{
      background: accent
        ? `linear-gradient(135deg, ${C.navyDk} 0%, ${C.navyMd} 100%)`
        : C.white,
      border: accent ? `1px solid ${C.navyMd}` : `1px solid ${C.border}`,
      borderRadius: 14, padding: "18px 20px",
      display:"flex", flexDirection:"column", gap:7,
      boxShadow: accent ? "0 8px 28px rgba(27,46,107,0.3)" : "0 2px 10px rgba(27,46,107,0.07)",
      position:"relative", overflow:"hidden",
    }}>
      {accent && <div style={{ position:"absolute", top:-20, right:-20, width:80, height:80, background:"rgba(245,168,0,0.1)", borderRadius:"50%" }}/>}
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
        <span style={{ fontSize:10, fontWeight:700, letterSpacing:"0.1em", textTransform:"uppercase", color: accent ? C.goldLt : C.slate }}>{label}</span>
        {icon && <span style={{ fontSize:18, opacity:0.8 }}>{icon}</span>}
      </div>
      <span style={{ fontSize:21, fontWeight:900, color: accent ? C.white : C.navy, lineHeight:1, fontFamily:"'Georgia',serif" }}>{value}</span>
      {sub  && <span style={{ fontSize:11, color: accent ? "rgba(255,255,255,0.6)" : C.slate }}>{sub}</span>}
      {sub2 && <span style={{ fontSize:11, color: accent ? C.goldLt : C.teal, fontWeight:600 }}>{sub2}</span>}
    </div>
  );
}

// ─── Badge ────────────────────────────────────────────────────────────────────
function Badge({ status }) {
  const map = {
    "Исполнен":   [C.green,"#E8F5E9"],
    "Оплачен":    [C.green,"#E8F5E9"],
    "В процессе": [C.gold, "#FFF8E1"],
    "Отменён":    [C.red,  "#FFEBEE"],
  };
  const [color,bg] = map[status]||[C.slate,"#F7F9FF"];
  return <span style={{ background:bg, color, fontSize:10, fontWeight:700, padding:"3px 10px", borderRadius:20, whiteSpace:"nowrap", border:`1px solid ${color}33` }}>{status}</span>;
}

// ─── Modal ────────────────────────────────────────────────────────────────────
function Modal({ title, onClose, children }) {
  return (
    <div style={{ position:"fixed", inset:0, background:"rgba(15,30,74,0.65)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }} onClick={onClose}>
      <div style={{ background:C.white, borderRadius:18, padding:"28px 26px", width:500, maxWidth:"100%", boxShadow:"0 32px 80px rgba(15,30,74,0.35)", maxHeight:"92vh", overflowY:"auto" }} onClick={e=>e.stopPropagation()}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:22 }}>
          <div style={{ display:"flex", alignItems:"center", gap:10 }}>
            <div style={{ width:4, height:22, background:C.gold, borderRadius:2 }}/>
            <h3 style={{ margin:0, color:C.navy, fontSize:16, fontWeight:800, fontFamily:"'Georgia',serif" }}>{title}</h3>
          </div>
          <button onClick={onClose} style={{ background:C.off, border:"none", width:30, height:30, borderRadius:8, fontSize:16, cursor:"pointer", color:C.slate }}>×</button>
        </div>
        {children}
      </div>
    </div>
  );
}

// ─── Input / Select ───────────────────────────────────────────────────────────
function Input({ label, hint, ...props }) {
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
      <div style={{ display:"flex", justifyContent:"space-between" }}>
        {label && <label style={{ fontSize:11, fontWeight:700, color:C.navy, letterSpacing:"0.04em" }}>{label}</label>}
        {hint  && <span style={{ fontSize:10, color:C.slate }}>{hint}</span>}
      </div>
      <input {...props} style={{ border:`1.5px solid ${C.border}`, borderRadius:9, padding:"9px 12px", fontSize:13, color:C.navy, outline:"none", background:C.off, transition:"border .15s", ...props.style }}
        onFocus={e=>e.target.style.borderColor=C.gold} onBlur={e=>e.target.style.borderColor=C.border}/>
    </div>
  );
}
function Sel({ label, children, ...props }) {
  return (
    <div style={{ display:"flex", flexDirection:"column", gap:4 }}>
      {label && <label style={{ fontSize:11, fontWeight:700, color:C.navy, letterSpacing:"0.04em" }}>{label}</label>}
      <select {...props} style={{ border:`1.5px solid ${C.border}`, borderRadius:9, padding:"9px 12px", fontSize:13, color:C.navy, outline:"none", background:C.off, ...props.style }}>{children}</select>
    </div>
  );
}

function Btn({ children, variant="primary", small, ...props }) {
  const s = {
    primary:   { background:C.gold, color:C.navyDk, border:"none", boxShadow:"0 4px 14px rgba(245,168,0,0.35)", fontWeight:800 },
    secondary: { background:C.white, color:C.navy, border:`1.5px solid ${C.border}`, fontWeight:700 },
    navy:      { background:C.navy, color:C.white, border:"none", fontWeight:700 },
  };
  return (
    <button {...props} style={{ ...s[variant], padding:small?"7px 14px":"10px 20px", borderRadius:9, fontSize:small?12:13, cursor:"pointer", transition:"all .15s", ...props.style }}
      onMouseOver={e=>e.currentTarget.style.opacity="0.87"} onMouseOut={e=>e.currentTarget.style.opacity="1"}>
      {children}
    </button>
  );
}

const TABS = [
  { id:"dashboard", label:"Дашборд",           icon:"📊" },
  { id:"income",    label:"Доходные договора",  icon:"📈" },
  { id:"expense",   label:"Расходные (тендер)", icon:"📋" },
  { id:"gph",       label:"Договоры ГПХ",       icon:"👤" },
  { id:"reports",   label:"Отчётность",         icon:"📑" },
];

// ═══════════════════════════════════════════════════════════════════════════════
export default function App() {
  const [tab,setTab]               = useState("dashboard");
  const [sidebarOpen,setSidebar]   = useState(true);
  const [incomeData,setIncomeData] = useState([]);
  const [expenseData,setExpenseData]= useState([]);
  const [gphData,setGphData]       = useState([]);
  const [modal,setModal]           = useState(null);
  const [filterYear,setFilterYear] = useState("all");
  const [reportYear,setReportYear] = useState(2026);

  const allExpenses    = [...expenseData,...gphData];
  const totalIncVat    = incomeData.reduce((s,r)=>s+r.amountWithVat,0);
  const totalIncNet    = incomeData.reduce((s,r)=>s+r.amountNet,0);
  const totalExpVat    = allExpenses.reduce((s,r)=>s+r.amountWithVat,0);
  const totalExpNet    = allExpenses.reduce((s,r)=>s+r.amountNet,0);
  const profit         = totalIncVat - totalExpVat;

  const fIncome  = filterYear==="all"?incomeData :incomeData.filter(r=>r.year===+filterYear);
  const fExpense = filterYear==="all"?expenseData:expenseData.filter(r=>r.year===+filterYear);
  const fGph     = filterYear==="all"?gphData    :gphData.filter(r=>r.year===+filterYear);

  const chartMonthly = useMemo(()=>MONTHS.map(m=>({
    month:m.slice(0,3),
    "Доходы":  incomeData.filter(r=>r.year===reportYear&&r.month===m).reduce((s,r)=>s+r.amountWithVat,0),
    "Расходы": [...expenseData,...gphData].filter(r=>r.year===reportYear&&r.month===m).reduce((s,r)=>s+r.amountWithVat,0),
  })),[incomeData,expenseData,gphData,reportYear]);

  const chartPie = useMemo(()=>{
    const t=expenseData.filter(r=>r.year===reportYear).reduce((s,r)=>s+r.amountWithVat,0);
    const g=gphData.filter(r=>r.year===reportYear).reduce((s,r)=>s+r.amountWithVat,0);
    return [{name:"Тендерные",value:t},{name:"ГПХ",value:g}];
  },[expenseData,gphData,reportYear]);

  const chartCompany = useMemo(()=>{
    const map={};
    incomeData.filter(r=>r.year===reportYear).forEach(r=>{map[r.company]=(map[r.company]||0)+r.amountWithVat;});
    return Object.entries(map).map(([name,value])=>({name:name.slice(0,14),value})).sort((a,b)=>b.value-a.value);
  },[incomeData,reportYear]);

  const chartProfit = useMemo(()=>MONTHS.map(m=>{
    const i=incomeData.filter(r=>r.year===reportYear&&r.month===m).reduce((s,r)=>s+r.amountWithVat,0);
    const e=[...expenseData,...gphData].filter(r=>r.year===reportYear&&r.month===m).reduce((s,r)=>s+r.amountWithVat,0);
    return {month:m.slice(0,3),"Прибыль":i-e};
  }),[incomeData,expenseData,gphData,reportYear]);

  const saveRecord=(type,item)=>{
    const n={...item,id:Date.now(),year:+item.year,amountWithVat:+item.amountWithVat,amountNet:+item.amountNet,vatAmount:+item.vatAmount};
    if(type==="income")setIncomeData(p=>[...p,n]);
    else if(type==="expense")setExpenseData(p=>[...p,n]);
    else setGphData(p=>[...p,n]);
    setModal(null);
  };
  const del=(type,id)=>{
    if(type==="income")setIncomeData(p=>p.filter(r=>r.id!==id));
    else if(type==="expense")setExpenseData(p=>p.filter(r=>r.id!==id));
    else setGphData(p=>p.filter(r=>r.id!==id));
  };

  return (
    <div style={{ fontFamily:"'Segoe UI',system-ui,sans-serif", background:"#EDF1FB", minHeight:"100vh", display:"flex" }}>

      {/* ── SIDEBAR ── */}
      <aside style={{
        width:sidebarOpen?240:0, minWidth:sidebarOpen?240:0, overflow:"hidden",
        background:`linear-gradient(180deg, ${C.navyDk} 0%, ${C.navy} 60%, ${C.navyMd} 100%)`,
        display:"flex", flexDirection:"column",
        transition:"width .3s cubic-bezier(.4,0,.2,1),min-width .3s cubic-bezier(.4,0,.2,1)",
        flexShrink:0, position:"sticky", top:0, height:"100vh",
        borderRight:`3px solid ${C.gold}`,
      }}>
        <div style={{ minWidth:240, display:"flex", flexDirection:"column", height:"100%" }}>

          {/* Logo block — centered, large */}
          <div style={{ padding:"28px 20px 20px", borderBottom:`1px solid rgba(245,168,0,0.25)`, display:"flex", flexDirection:"column", alignItems:"center", gap:14, background:"rgba(0,0,0,0.18)" }}>
            {/* Logo image centered and large */}
            <div style={{ width:110, height:110, display:"flex", alignItems:"center", justifyContent:"center", background:"#FFFFFF", borderRadius:16, padding:10, border:`2px solid ${C.gold}`, boxShadow:`0 4px 24px rgba(0,0,0,0.35), 0 0 0 4px rgba(245,168,0,0.15)` }}>
              <img src={LOGO_B64} alt="KNU" style={{ width:"100%", height:"100%", objectFit:"contain" }}/>
            </div>
            {/* Text under logo */}
            <div style={{ textAlign:"center" }}>
              <div style={{ color:C.white, fontFamily:"'Georgia',serif", fontWeight:700, fontSize:11, letterSpacing:"0.08em", lineHeight:1.5, textTransform:"uppercase" }}>Kazakhstan</div>
              <div style={{ color:C.gold, fontFamily:"'Georgia',serif", fontWeight:800, fontSize:13, letterSpacing:"0.06em", lineHeight:1.4, textTransform:"uppercase" }}>Nuclear</div>
              <div style={{ color:C.white, fontFamily:"'Georgia',serif", fontWeight:700, fontSize:11, letterSpacing:"0.08em", lineHeight:1.5, textTransform:"uppercase" }}>University</div>
            </div>
            {/* Gold divider line */}
            <div style={{ width:40, height:2, background:`linear-gradient(90deg, transparent, ${C.gold}, transparent)`, borderRadius:1 }}/>
            <div style={{ color:"rgba(255,255,255,0.45)", fontSize:9, fontWeight:700, letterSpacing:"0.14em", textTransform:"uppercase" }}>Финансовый мониторинг</div>
          </div>

          {/* Nav */}
          <nav style={{ flex:1, padding:"14px 10px", overflowY:"auto" }}>
            {TABS.map(t=>(
              <button key={t.id} onClick={()=>setTab(t.id)} style={{
                display:"flex", alignItems:"center", gap:10, width:"100%",
                padding:"11px 13px", borderRadius:10, border:"none", cursor:"pointer", marginBottom:3,
                background:tab===t.id?`rgba(245,168,0,0.18)`:"transparent",
                color:tab===t.id?C.gold:"#8899BB",
                fontWeight:tab===t.id?700:500, fontSize:13, textAlign:"left",
                transition:"all .15s", whiteSpace:"nowrap",
                borderLeft:tab===t.id?`3px solid ${C.gold}`:"3px solid transparent",
              }}>
                <span style={{ fontSize:15, flexShrink:0 }}>{t.icon}</span>
                <span>{t.label}</span>
              </button>
            ))}
          </nav>

          {/* Bottom user block */}
          <div style={{ padding:"12px 14px 18px" }}>
            <div style={{ background:"rgba(245,168,0,0.12)", border:`1px solid rgba(245,168,0,0.25)`, borderRadius:10, padding:"11px 13px" }}>
              <div style={{ color:C.gold, fontSize:9, fontWeight:800, letterSpacing:"0.1em" }}>РУКОВОДИТЕЛЬ</div>
              <div style={{ color:C.white, fontSize:12, fontWeight:700, marginTop:2 }}>Панель управления</div>
            </div>
          </div>
        </div>
      </aside>

      {/* ── TOGGLE ── */}
      <button onClick={()=>setSidebar(p=>!p)} style={{
        position:"fixed", bottom:24, left:sidebarOpen?198:12, zIndex:200,
        width:32, height:32, background:C.navy, border:`2px solid ${C.gold}`,
        borderRadius:"50%", cursor:"pointer", display:"flex", alignItems:"center", justifyContent:"center",
        boxShadow:"0 4px 16px rgba(15,30,74,0.4)", transition:"left .3s cubic-bezier(.4,0,.2,1)",
        color:C.gold, fontSize:12, fontWeight:800,
      }}>{sidebarOpen?"◀":"▶"}</button>

      {/* ── MAIN ── */}
      <main style={{ flex:1, overflow:"auto", minWidth:0 }}>

        {/* Top bar */}
        <div style={{ background:C.white, borderBottom:`1px solid ${C.border}`, padding:"12px 28px", display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:10, boxShadow:"0 2px 8px rgba(27,46,107,0.06)" }}>
          <div style={{ display:"flex", alignItems:"center", gap:12 }}>
            <div style={{ width:3, height:18, background:C.gold, borderRadius:2 }}/>
            <span style={{ color:C.navy, fontWeight:800, fontSize:15, fontFamily:"'Georgia',serif" }}>
              {TABS.find(t=>t.id===tab)?.label}
            </span>
          </div>
          <KNULogoMini/>
        </div>

        <div style={{ padding:"24px 28px 60px" }}>

        {/* ════ DASHBOARD ════ */}
        {tab==="dashboard" && (
          <section>
            <div style={{ marginBottom:22 }}>
              <h1 style={{ color:C.navy, margin:"0 0 3px", fontSize:20, fontWeight:900, fontFamily:"'Georgia',serif" }}>Общий дашборд</h1>
              <p style={{ color:C.slate, margin:0, fontSize:12 }}>Финансовый обзор по всем договорам</p>
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(200px,1fr))", gap:14, marginBottom:24 }}>
              <KpiCard accent icon="📈" label="Доход с НДС"
                value={totalIncVat>0?fmt(totalIncVat):"—"}
                sub={totalIncNet>0?`Без НДС: ${fmt(totalIncNet)}`:`${incomeData.length} договоров`}
                sub2={totalIncVat>0?`НДС: ${fmt(totalIncVat-totalIncNet)}`:undefined}/>
              <KpiCard icon="📉" label="Расход с НДС"
                value={totalExpVat>0?fmt(totalExpVat):"—"}
                sub={totalExpNet>0?`Без НДС: ${fmt(totalExpNet)}`:undefined}
                sub2={totalExpVat>0?`НДС: ${fmt(totalExpVat-totalExpNet)}`:undefined}/>
              <KpiCard icon="💰" label="Прибыль"
                value={totalIncVat>0||totalExpVat>0?fmt(profit):"—"}
                sub={profit>=0?"Положительный баланс":"Дефицит"}/>
              <KpiCard icon="👤" label="Договоры ГПХ"
                value={gphData.length||"0"}
                sub={gphData.length>0?`С НДС: ${fmt(gphData.reduce((s,r)=>s+r.amountWithVat,0))}`:undefined}/>
            </div>
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:18 }}>
              <InfoCard title="Последние доходные договора">
                {incomeData.length===0?<EmptyState text="Доходных договоров ещё нет"/>
                  :incomeData.slice(-5).reverse().map(r=>(
                    <DRow key={r.id} top={r.company} bot={`${r.month} ${r.year} · ${r.description}`}
                      vat={r.amountWithVat} net={r.amountNet} status={r.status}/>
                  ))}
              </InfoCard>
              <InfoCard title="Последние расходы">
                {allExpenses.length===0?<EmptyState text="Расходов ещё нет"/>
                  :allExpenses.slice(-5).reverse().map(r=>(
                    <DRow key={r.id} top={r.person||r.company} bot={`${r.month} ${r.year} · ${r.description}`}
                      vat={r.amountWithVat} net={r.amountNet} status={r.status} red/>
                  ))}
              </InfoCard>
            </div>
          </section>
        )}

        {/* ════ INCOME ════ */}
        {tab==="income" && (
          <TableSec title="Доходные договора" subtitle="Договоры с постоянными партнёрами"
            rows={fIncome} filterYear={filterYear} setFilterYear={setFilterYear}
            onAdd={()=>setModal({type:"income"})} onDelete={id=>del("income",id)}
            nameCol="company" nameLabel="Компания"/>
        )}

        {/* ════ EXPENSE ════ */}
        {tab==="expense" && (
          <TableSec title="Расходные договора (тендер)" subtitle="Открытые тендеры"
            rows={fExpense} filterYear={filterYear} setFilterYear={setFilterYear}
            onAdd={()=>setModal({type:"expense"})} onDelete={id=>del("expense",id)}
            nameCol="company" nameLabel="Поставщик"/>
        )}

        {/* ════ GPH ════ */}
        {tab==="gph" && (
          <TableSec title="Договоры ГПХ" subtitle="Гражданско-правовые договоры с физ. лицами"
            rows={fGph} filterYear={filterYear} setFilterYear={setFilterYear}
            onAdd={()=>setModal({type:"gph"})} onDelete={id=>del("gph",id)}
            nameCol="person" nameLabel="Физ. лицо"/>
        )}

        {/* ════ REPORTS ════ */}
        {tab==="reports" && (
          <section>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:22, flexWrap:"wrap", gap:12 }}>
              <div>
                <h1 style={{ color:C.navy, margin:"0 0 3px", fontSize:20, fontWeight:900, fontFamily:"'Georgia',serif" }}>Отчётность</h1>
                <p style={{ color:C.slate, margin:0, fontSize:12 }}>Аналитика по договорам и финансам</p>
              </div>
              <Sel value={reportYear} onChange={e=>setReportYear(+e.target.value)} style={{ width:116 }}>
                {YEARS.map(y=><option key={y} value={y}>{y}</option>)}
              </Sel>
            </div>

            {incomeData.length===0&&expenseData.length===0&&gphData.length===0?(
              <div style={{ background:C.white, border:`2px dashed ${C.border}`, borderRadius:16, padding:"60px 40px", textAlign:"center" }}>
                <div style={{ fontSize:44, marginBottom:14 }}>📊</div>
                <div style={{ color:C.navy, fontWeight:800, fontSize:17, marginBottom:8, fontFamily:"'Georgia',serif" }}>Нет данных для отчёта</div>
                <div style={{ color:C.slate, fontSize:13 }}>Добавьте договора, чтобы здесь появились графики</div>
              </div>
            ):(<>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:18, marginBottom:18 }}>
                <ChCard title={`Доходы и расходы (с НДС) — ${reportYear}`}>
                  <ResponsiveContainer width="100%" height={230}>
                    <BarChart data={chartMonthly} margin={{top:4,right:8,left:0,bottom:0}}>
                      <CartesianGrid strokeDasharray="3 3" stroke={C.border}/>
                      <XAxis dataKey="month" tick={{fontSize:10,fill:C.slate}}/>
                      <YAxis tick={{fontSize:10,fill:C.slate}} tickFormatter={v=>v>=1e6?(v/1e6).toFixed(1)+"M":v>=1000?(v/1000)+"K":v}/>
                      <Tooltip formatter={v=>fmt(v)}/>
                      <Legend/>
                      <Bar dataKey="Доходы"  fill={C.navy} radius={[4,4,0,0]}/>
                      <Bar dataKey="Расходы" fill={C.gold} radius={[4,4,0,0]}/>
                    </BarChart>
                  </ResponsiveContainer>
                </ChCard>
                <ChCard title="Структура расходов">
                  <ResponsiveContainer width="100%" height={230}>
                    <PieChart>
                      <Pie data={chartPie} cx="50%" cy="50%" outerRadius={85} dataKey="value"
                        label={({name,percent})=>`${name} ${(percent*100).toFixed(0)}%`}>
                        <Cell fill={C.navy}/><Cell fill={C.gold}/>
                      </Pie>
                      <Tooltip formatter={v=>fmt(v)}/>
                    </PieChart>
                  </ResponsiveContainer>
                </ChCard>
              </div>
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:18 }}>
                <ChCard title="Топ клиенты по доходам">
                  {chartCompany.length===0?<EmptyState text={`Нет данных за ${reportYear}`}/>
                    :<ResponsiveContainer width="100%" height={200}>
                      <BarChart data={chartCompany} layout="vertical" margin={{top:0,right:8,left:8,bottom:0}}>
                        <CartesianGrid strokeDasharray="3 3" stroke={C.border} horizontal={false}/>
                        <XAxis type="number" tick={{fontSize:10,fill:C.slate}} tickFormatter={v=>v>=1e6?(v/1e6).toFixed(1)+"M":(v/1000)+"K"}/>
                        <YAxis type="category" dataKey="name" tick={{fontSize:11,fill:C.navy}} width={90}/>
                        <Tooltip formatter={v=>fmt(v)}/>
                        <Bar dataKey="value" fill={C.navyMd} radius={[0,4,4,0]}/>
                      </BarChart>
                    </ResponsiveContainer>
                  }
                </ChCard>
                <ChCard title="Прибыль по месяцам">
                  <ResponsiveContainer width="100%" height={200}>
                    <LineChart data={chartProfit} margin={{top:4,right:8,left:0,bottom:0}}>
                      <CartesianGrid strokeDasharray="3 3" stroke={C.border}/>
                      <XAxis dataKey="month" tick={{fontSize:10,fill:C.slate}}/>
                      <YAxis tick={{fontSize:10,fill:C.slate}} tickFormatter={v=>v>=1e6?(v/1e6).toFixed(1)+"M":v>=1000?(v/1000)+"K":v}/>
                      <Tooltip formatter={v=>fmt(v)}/>
                      <Line type="monotone" dataKey="Прибыль" stroke={C.gold} strokeWidth={2.5} dot={{fill:C.gold,r:3}}/>
                    </LineChart>
                  </ResponsiveContainer>
                </ChCard>
              </div>
            </>)}
          </section>
        )}
        </div>
      </main>

      {modal && <AddModal type={modal.type} onClose={()=>setModal(null)} onSave={saveRecord}
        incomeData={incomeData} expenseData={expenseData} gphData={gphData}/>}
    </div>
  );
}

// ─── Info Card ────────────────────────────────────────────────────────────────
function InfoCard({ title, children }) {
  return (
    <div style={{ background:C.white, border:`1px solid ${C.border}`, borderRadius:14, padding:"18px 18px 6px", boxShadow:"0 2px 10px rgba(27,46,107,0.06)" }}>
      <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:12 }}>
        <div style={{ width:3, height:14, background:C.gold, borderRadius:2 }}/>
        <div style={{ fontWeight:800, color:C.navy, fontSize:12, fontFamily:"'Georgia',serif" }}>{title}</div>
      </div>
      {children}
    </div>
  );
}
function DRow({ top, bot, vat, net, status, red }) {
  return (
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", padding:"9px 0", borderBottom:`1px solid ${C.border}` }}>
      <div style={{ flex:1, minWidth:0, marginRight:8 }}>
        <div style={{ fontSize:12, fontWeight:600, color:C.navy, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{top}</div>
        <div style={{ fontSize:10, color:C.slate, marginTop:1 }}>{bot}</div>
      </div>
      <div style={{ textAlign:"right", flexShrink:0 }}>
        <div style={{ fontSize:12, fontWeight:800, color:red?C.red:C.navy }}>{red?"−":""}{num(vat)} ₸</div>
        <div style={{ fontSize:10, color:C.teal }}>б/НДС: {num(net)} ₸</div>
        <Badge status={status}/>
      </div>
    </div>
  );
}
function EmptyState({ text }) {
  return <div style={{ padding:"24px 12px", textAlign:"center", color:C.slate, fontSize:12 }}><div style={{ fontSize:26,marginBottom:6 }}>📭</div>{text}</div>;
}
function ChCard({ title, children }) {
  return (
    <div style={{ background:C.white, border:`1px solid ${C.border}`, borderRadius:14, padding:"18px 18px 14px", boxShadow:"0 2px 10px rgba(27,46,107,0.06)" }}>
      <div style={{ display:"flex", alignItems:"center", gap:7, marginBottom:12 }}>
        <div style={{ width:3, height:13, background:C.gold, borderRadius:2 }}/>
        <div style={{ fontWeight:800, color:C.navy, fontSize:12, fontFamily:"'Georgia',serif" }}>{title}</div>
      </div>
      {children}
    </div>
  );
}

// ─── Table Section ────────────────────────────────────────────────────────────
function TableSec({ title, subtitle, rows, filterYear, setFilterYear, onAdd, onDelete, nameCol, nameLabel }) {
  const totVat = rows.reduce((s,r)=>s+r.amountWithVat,0);
  const totNet = rows.reduce((s,r)=>s+r.amountNet,0);
  return (
    <section>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:18, flexWrap:"wrap", gap:12 }}>
        <div>
          <h1 style={{ color:C.navy, margin:"0 0 3px", fontSize:20, fontWeight:900, fontFamily:"'Georgia',serif" }}>{title}</h1>
          <p style={{ color:C.slate, margin:0, fontSize:12 }}>{subtitle}</p>
        </div>
        <div style={{ display:"flex", gap:10, alignItems:"center" }}>
          <Sel value={filterYear} onChange={e=>setFilterYear(e.target.value)} style={{ width:106 }}>
            <option value="all">Все годы</option>
            {YEARS.map(y=><option key={y} value={y}>{y}</option>)}
          </Sel>
          <Btn onClick={onAdd}>+ Добавить</Btn>
        </div>
      </div>

      {/* Stats strip */}
      <div style={{ background:`linear-gradient(135deg,${C.navyDk} 0%,${C.navyMd} 100%)`, borderRadius:12, padding:"14px 20px", marginBottom:16, display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))", gap:14, boxShadow:"0 6px 20px rgba(15,30,74,0.22)", borderLeft:`4px solid ${C.gold}` }}>
        {[["Записей",rows.length,"#8FAAD4"],["Сумма с НДС",rows.length>0?fmt(totVat):"—",C.gold],["Без НДС",rows.length>0?fmt(totNet):"—",C.goldLt],["НДС 12%",rows.length>0?fmt(totVat-totNet):"—","#A78BFA"]].map(([l,v,col])=>(
          <div key={l}>
            <div style={{ color:"#5570A0", fontSize:9, fontWeight:700, letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:3 }}>{l}</div>
            <div style={{ fontSize:15, fontWeight:900, color:col, fontFamily:"'Georgia',serif" }}>{v}</div>
          </div>
        ))}
      </div>

      {rows.length===0?(
        <div style={{ background:C.white, border:`2px dashed ${C.border}`, borderRadius:14, padding:"44px 24px", textAlign:"center" }}>
          <div style={{ fontSize:32,marginBottom:8 }}>📝</div>
          <div style={{ color:C.navy, fontWeight:700, fontSize:15, marginBottom:6, fontFamily:"'Georgia',serif" }}>Нет записей</div>
          <div style={{ color:C.slate, fontSize:12, marginBottom:18 }}>Нажмите «+ Добавить» чтобы внести первый договор</div>
          <Btn onClick={onAdd}>+ Добавить первую запись</Btn>
        </div>
      ):(
        <div style={{ background:C.white, border:`1px solid ${C.border}`, borderRadius:14, overflow:"hidden", boxShadow:"0 2px 10px rgba(27,46,107,0.06)" }}>
          <div style={{ overflowX:"auto" }}>
            <table style={{ width:"100%", borderCollapse:"collapse", minWidth:680 }}>
              <thead>
                <tr style={{ background:"#EEF2FF" }}>
                  {[nameLabel,"Год","Месяц","С НДС (₸)","Без НДС (₸)","НДС 12% (₸)","Описание","Статус"].map(h=>(
                    <th key={h} style={{ padding:"10px 14px", textAlign:"left", fontSize:9, fontWeight:800, color:C.slate, letterSpacing:"0.08em", textTransform:"uppercase", borderBottom:`1.5px solid ${C.border}`, whiteSpace:"nowrap" }}>{h}</th>
                  ))}
                  <th style={{ padding:"10px 14px", borderBottom:`1.5px solid ${C.border}` }}/>
                </tr>
              </thead>
              <tbody>
                {rows.map((r,i)=>(
                  <tr key={r.id} style={{ borderBottom:`1px solid ${C.border}`, background:i%2===0?C.white:C.off }}>
                    <td style={{ padding:"10px 14px", fontSize:12, color:C.navy, fontWeight:600, whiteSpace:"nowrap" }}>{r[nameCol]}</td>
                    <td style={{ padding:"10px 14px", fontSize:12, color:C.navy }}>{r.year}</td>
                    <td style={{ padding:"10px 14px", fontSize:12, color:C.navy, whiteSpace:"nowrap" }}>{r.month}</td>
                    <td style={{ padding:"10px 14px", fontSize:12, fontWeight:800, color:C.navy, whiteSpace:"nowrap" }}>{num(r.amountWithVat)} ₸</td>
                    <td style={{ padding:"10px 14px", fontSize:12, fontWeight:700, color:C.teal, whiteSpace:"nowrap" }}>{num(r.amountNet)} ₸</td>
                    <td style={{ padding:"10px 14px", fontSize:12, color:"#7C3AED", whiteSpace:"nowrap" }}>{num(r.vatAmount)} ₸</td>
                    <td style={{ padding:"10px 14px", fontSize:12, color:C.slate, minWidth:120 }}>{r.description}</td>
                    <td style={{ padding:"10px 14px" }}><Badge status={r.status}/></td>
                    <td style={{ padding:"10px 14px" }}>
                      <button onClick={()=>onDelete(r.id)} style={{ background:"#FFEBEE", color:C.red, border:"none", borderRadius:7, padding:"4px 10px", fontSize:11, fontWeight:700, cursor:"pointer" }}>✕</button>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </div>
      )}
    </section>
  );
}

// ─── Add Modal ────────────────────────────────────────────────────────────────
function AddModal({ type, onClose, onSave, incomeData, expenseData, gphData }) {
  const isGph=type==="gph", isIncome=type==="income";
  const nextLabel=()=>{
    if(isGph)    return `Физ. лицо #${gphData.length+1}`;
    if(isIncome) return `Компания #${incomeData.length+1}`;
    return `Поставщик #${expenseData.length+1}`;
  };
  const [form,setForm]=useState({
    year:2026,month:"Январь",description:"",status:"В процессе",
    company:nextLabel(),person:nextLabel(),
    amountWithVat:"",amountNet:"",vatAmount:"",
  });
  const set=(k,v)=>setForm(p=>({...p,[k]:v}));
  const handleAmt=(field,val)=>{
    const n=parseFloat(val)||0;
    if(field==="amountWithVat") setForm(p=>({...p,amountWithVat:val,amountNet:(withoutVat(n)).toFixed(2),vatAmount:(vatOnly(n)).toFixed(2)}));
    else setForm(p=>({...p,amountNet:val,amountWithVat:(withVat(n)).toFixed(2),vatAmount:(withVat(n)-n).toFixed(2)}));
  };
  const titles={income:"Добавить доходный договор",expense:"Добавить расходный договор",gph:"Добавить договор ГПХ"};

  return (
    <Modal title={titles[type]} onClose={onClose}>
      <div style={{ display:"flex",flexDirection:"column",gap:13 }}>
        {isGph
          ?<Input label="ФИО физ. лица" value={form.person} onChange={e=>set("person",e.target.value)} placeholder="Физ. лицо #1"/>
          :<Input label={isIncome?"Название компании":"Поставщик / Подрядчик"} value={form.company} onChange={e=>set("company",e.target.value)} placeholder="Компания #1"/>
        }
        <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:11 }}>
          <Sel label="Год" value={form.year} onChange={e=>set("year",e.target.value)}>
            {YEARS.map(y=><option key={y} value={y}>{y}</option>)}
          </Sel>
          <Sel label="Месяц" value={form.month} onChange={e=>set("month",e.target.value)}>
            {MONTHS.map(m=><option key={m} value={m}>{m}</option>)}
          </Sel>
        </div>

        {/* Amount block */}
        <div style={{ background:C.off, border:`1.5px solid ${C.border}`, borderRadius:11, padding:"13px 13px 10px" }}>
          <div style={{ fontSize:10, fontWeight:800, color:C.navy, marginBottom:10, letterSpacing:"0.05em", display:"flex", alignItems:"center", gap:6 }}>
            <div style={{ width:3, height:12, background:C.gold, borderRadius:2 }}/>
            СУММА ДОГОВОРА
          </div>
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:11,marginBottom:9 }}>
            <Input label="С НДС (₸)" hint="авто-расчёт" type="number" value={form.amountWithVat} placeholder="0" onChange={e=>handleAmt("amountWithVat",e.target.value)}/>
            <Input label="Без НДС (₸)" hint="или введите здесь" type="number" value={form.amountNet} placeholder="0" onChange={e=>handleAmt("amountNet",e.target.value)}/>
          </div>
          <div style={{ background:"rgba(124,58,237,0.08)", borderRadius:8, padding:"7px 12px", display:"flex", justifyContent:"space-between", alignItems:"center", border:"1px solid rgba(124,58,237,0.15)" }}>
            <span style={{ fontSize:11, color:"#7C3AED", fontWeight:700 }}>НДС 12%</span>
            <span style={{ fontSize:13, fontWeight:800, color:"#7C3AED" }}>{form.vatAmount?num(+form.vatAmount)+" ₸":"—"}</span>
          </div>
        </div>

        <Input label="Описание / Предмет договора" value={form.description} onChange={e=>set("description",e.target.value)} placeholder="Краткое описание"/>
        <Sel label="Статус" value={form.status} onChange={e=>set("status",e.target.value)}>
          <option>В процессе</option><option>Исполнен</option><option>Оплачен</option><option>Отменён</option>
        </Sel>
        <div style={{ display:"flex",gap:10,justifyContent:"flex-end",marginTop:4 }}>
          <Btn variant="secondary" onClick={onClose}>Отмена</Btn>
          <Btn onClick={()=>{
            if(!form.amountWithVat&&!form.amountNet)return;
            onSave(type,{...form,type:isIncome?"income":"expense",subtype:isGph?"gph":"tender",amountWithVat:+form.amountWithVat||0,amountNet:+form.amountNet||0,vatAmount:+form.vatAmount||0});
          }}>Сохранить</Btn>
        </div>
      </div>
    </Modal>
  );
}
